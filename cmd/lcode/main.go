...
func printUsage() {
	...
	fmt.Println()
	fmt.Println("用法:")
	fmt.Println("  lcode studio [file]          启动全套高颜值终端编译器 Studio (默认 examples/hello.lc)")
	fmt.Println("  lcode repl                   启动交互式命令行 REPL (支持按 :c 补全)")
	fmt.Println("  lcode run <file>             编译并执行源程序 (自动解析模块导入)")
	fmt.Println("  lcode to-cpp <file> [-o out] 将源文件转译为现代 C++17 源码并调用内置 C++ 库")
	fmt.Println("  lcode complete <prefix>      智能代码自动补全建议查询 (支持 --json 格式)")
	fmt.Println("  lcode init                   初始化当前目录为 Lcode 项目 (生成 lcode.toml)")
	fmt.Println("  lcode ast <file>             以彩色树形分支结构输出抽象语法树 (AST)")
	fmt.Println("  lcode tokenize <file>        以美化表格输出词法单元 (Token) 流")
	fmt.Println("  lcode check <file>           执行编译前端全流程检查与精准代码诊断")
	fmt.Println("  lcode version                显示版本信息")
	fmt.Println("  lcode help                   显示此帮助信息")
}
...
`
	...
	fmt.Println(ui.Paint(ui.ColorMuted, "运行项目: lcode run main.lc"))
}
